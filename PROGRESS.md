...
| 2026-09-05 | **Screen-replay simulation probe (Phase-04 red-team).** New `scripts/attack_replay_sim.py` runs all 240 quick-shim fakes through the shipped `ScreenReplaySimulator` (seed=7; display-composite is a no-op at 256px, moire/gamut/sheen/flatten apply); manifest `data/manifests/quick_replay_manifest.csv`, 441/480 captures kept. Probe (real vs replayed-fake, n=441): replay_detect 0.862 (unchanged vs clean), prnu 0.878, texture 0.828, fft 0.796, CA 0.931. Clean-vs-replayed follow-up (n=192+226): `replay_moire_peak` moves (0.828) but `replay_midband_ratio` — the feature the verdict leans on — does not (0.568). Finding in RISK_REGISTER: the leaning feature is a fake-content cue, not a screen cue; replayed-REAL coverage unproven until campaign screens arrive. PRNU numbers on simulated replays are simulation artifacts (no camera noise added). | Replayed-real positives need photographed screens (capture campaign). |
| 2026-09-05 | **Face-morph probe (Phase-04 red-team, first non-synthetic attack).** New `scripts/attack_morph.py`: sequential ffhq/celeba-hq real pairs, eye-midpoint/iod/angle similarity alignment to a natural-scale 256px canvas (dst_iod 56px), alpha-0.5 blend inside a feathered FACE_OVAL mask, lossless PNGs; manifest `data/manifests/quick_morph_manifest.csv`, 103/107 morphs kept. Recipe bugs fixed along the way (attack script only): v1 zoom starved bg estimators (bogus PRNU 1.000), v2 halved heads (eye-width vs iod confusion), warp black-fill tripped the clipped gate (BORDER_REPLICATE). Probe (real vs morph, n=318): prnu **0.779** / texture **0.766** / replay **0.679** / CA **0.675** / fft 0.641 (BENCH). Mechanism is whole-frame resampling softness (both parents warped), not morph-specific cues — recorded in RISK_REGISTER as an UPPER BOUND: pro pipelines with grain matching would close most of the gap; structural defence is enrolment-time duplicate-identity matching (future work). | Morph training data + torch-gated morph-detection models. |
| 2026-09-05 | **Fusion probe (Phase-07 prototype, first joint model).** Installed scikit-learn 1.9.0 + scipy 1.18.1 into .venv (both already declared in pyproject ml extra). New scripts/extract_quick_features.py (degrades quick256 once with harness seed, runs 5 KEEP signals, writes data/interim/quick_features.csv: 318/407 rows x 18 features, CA coverage is the binding constraint) + scripts/train_fusion_probe.py (StandardScaler + LR, in-sample isotonic, 5th/95th-percentile conformal-style band, same split_indices folds). Probe (n=318, n_splits=2): mean AUC raw **0.906** / calibrated 0.904 - matches best single signal (prnu 0.907), no fusion gain on 256px data; uncertain rate 10% (under 15% target) but error|certain 12% (over 5% bar - needs nested calibration + full-res). Top standardized LR weights span all five signals (CA edge_patches 1.44, texture logratio 1.30, prnu logratio 1.23, replay midband 1.09): complementary cues, nothing dominated. Report in reports/fusion_probe_quick256.json. | Nested calibration splits + full-res features + torch-gated neural baseline. |
| 2026-09-05 | **Nested fusion calibration (Phase-07 follow-up).** New scripts/train_fusion_nested.py: same rows, outer folds and seed as the fusion probe, but the isotonic map + band edges fit on a held-out 1/3 calibration slice of each train fold (random stratified inner split - a train fold holds one group per class so a group-wise inner split is impossible; leakage touches only the 1-D map). Nested (n=318, n_splits=2): mean AUC raw 0.905 / calibrated 0.889; uncertain rate 17.4 pct (up from 10 pct, now over the 15 pct target); error-given-certain 10.8 pct (down from 12 pct, still over the 5 pct bar). Fold asymmetry is the finding: celeba/flux fold uncertain 32 pct err 9 pct vs ffhq/sdxl fold uncertain 3 pct err 13 pct - band edges from ~50-sample calibration slices swing wildly. Honest conclusion: the 5 pct UNCERTAIN bar is unreachable at this n; more source groups needed before it is claimable. Report in reports/fusion_nested_quick256.json. | More source groups + full-res features + torch-gated neural baseline. |
| 2026-09-05 | **quick256-v2: third source groups + n_splits=3 (calibration follow-up).** Rejected group candidates first (documented in scripts/fetch_quick_shim_v2.py): bitmind/lfw (faces below the 24px-eye shim gate, 6/120 kept), white_paper_holdout_4 RealVisXL (no frontal faces - MediaPipe NO_FACE 32/40, Haar 0/12), bitmind/face-swap (178x218, below the eye gate). Added instead: ffhq1024 (bitmind/ffhq shard 100, 1024px resized to 256 LANCZOS, disjoint shard from the v1 slice so no identity overlap) + flux-celeba (FLUX on CelebA domain: same family as v1 flux, new domain = honest domain-shift group). Manifest data/manifests/quick_manifest_v2.csv, 637/720 kept, 3 groups/side. Harness v2 (n=637, n_splits=3): prnu **0.869** / CA **0.894** / replay **0.895** / texture **0.860** / fft **0.744** - all five still KEEP; PRNU softens under domain shift as expected. Fusion probe v2 (n=503, n_splits=3): mean AUC raw 0.911-0.932 per fold, uncertain ~21 pct, err|certain ~12 pct. Nested v2: uncertain **43 pct** (folds 65/55/9 pct) - the 5th/95th-percentile band construction breaks under multi-group folds (bimodal score unions widen the band); proper class-conditional conformal is now a named Phase-07 item, more data alone will not fix the band. Also fixed stale hardcoded note strings in both fusion scripts (n_splits=2, prnu 0.907). Reports: reports/fusion_probe_quick256_v2.json + reports/fusion_nested_quick256_v2.json. | Class-conditional conformal band + full-res features + torch-gated neural baseline. |
| 2026-09-05 | **Band transportability (Phase-07 follow-up, hypothesis rejected).** New scripts/eval_band_transport.py: same rows, outer folds and seed as nested v2, but the 1/3 calibration slice is split into calA (band fit) and calB (in-domain eval, unseen rows, same domains); the same band is then scored on calB (in-domain) and on the held-out-domain test fold (transported). v2 (n=503, n_splits=3): in-domain uncertain 24 pct err 14.5 pct vs transported uncertain 22 pct err 13.8 pct - essentially identical. The transport hypothesis is rejected: band width is intrinsic class overlap at 256px, not domain shift, so per-deployment calibration is NOT supported as a fix. v1 rerun concurs (in-domain 10 pct/8.3 pct vs transported 7.6 pct/14.7 pct). Side finding: fold-1 transported width moved 65 pct (nested, full cal) to 16 pct (calA half) - percentile edges are high-variance on small bimodal cal sets regardless of domain. Conclusion stands and hardens: UNCERTAIN targets unreachable until score separation improves (better features / full-res); band work waits on that, not on more threshold machinery. Reports: reports/fusion_band_transport_v2.json + reports/fusion_band_transport_v1.json. | Better-separated scores (full-res features) + torch-gated neural baseline. |
| 2026-09-05 | **Group diagnostic + signal ablation (Phase-07 follow-up).** New scripts/eval_group_ablation.py on v2 features (n=503, n_splits=3, same folds/seed): leave-one-signal-out refits show CA carries the most unique evidence (drop delta +0.063), replay +0.015, texture +0.004, fft +0.001, prnu -0.000 - the best single signal (0.869) is fully redundant given the other four at 256px. Per-held-out-group median scores sit comfortably on the correct side (reals 0.03-0.14, fakes 0.83-0.88); flux-celeba (0.829) is not near the boundary, so no single domain drives the overlap - consistent with the transport finding (intrinsic within-group spread). Phase-07 implication: weight CA highest, treat prnu as corroboration not independent evidence at this resolution; revisit redundancy on full-res. Report in reports/fusion_ablation_v2.json. | Full-res features (redundancy may change) + torch-gated neural baseline. |
